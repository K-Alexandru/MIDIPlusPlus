...
declared a file-scope `POINT_COUNT = 33` that was shadowed inside the class by
`POINT_COUNT = 32`, and the loop at line 100 runs `i <= POINT_COUNT`. The dead
constant was removed on 2026-09-15; the `<=` loops draw grid lines, one more
than there are points, and are right.
...
than a tuning question. **Fixed 2026-09-15:** `MIDI++/VelocityPresets.hpp`
builds the presets strictly increasing from 1 and ending at 127, and
`VelocityFrame.cpp` loads them from there. `VelocityPresetTests` and the
`preset-floor-at-zero` mutation hold it.
