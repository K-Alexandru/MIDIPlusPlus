...
Everything is in `ui/`. The findings were written before any of it was fixed;
the Status section below records what happened to them.

## Status

Every item below was addressed on `astra/panel-spec-pass`, implemented by GPT
Astra and finished here. The findings are kept as written so the reasoning
survives; each carries what was done. Rendered pairs were re-read afterwards
at 100, 150 and 200 percent and back to 100.
...
## Not ported at the time, now ported
...
`astra/panel-spec-pass`, on top of this report:

- All three defects fixed: `Ellipsis` offsets by the line's text base offset,
  sustain cutoff goes through `Groove`, mini transpose uses `Icon::Minus` and
  `Icon::Plus`.
- The strip is the spec's five pills. MidiConnect has no action in this shell,
  so its pill reads as unavailable with a tooltip rather than sitting there as
  an inert toggle.
- Items 6 to 12 are done: sort, sizes and a selection bar in the file list,
  uppercase headers with MUTE SOLO spanning both button columns, hairline
  status separators, a segmented Live/Autoplay with its Curve label, uppercase
  Settings labels with described switches, amber black keys and an en dash.
  Velocity hotkeys and Always on top are wired and saved.
- Two deviations the owner asked for after seeing the renders: no underline on
  an on pill, and one drawing path for every pill so the labels sit evenly.
  On is still marked by weight as well as colour.
- The icons are Lucide now, generated by `tools/gen-icons.py` rather than drawn
