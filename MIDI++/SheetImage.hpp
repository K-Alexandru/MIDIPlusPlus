...
// SheetImage: the coloured sheet as a PNG.
//
// Sheets are shared as pictures as often as text, and the editor page can
// only save what a browser saves. This draws a StyledResult the way the page
// shows it, midi-converter's dark page and Verdana with the rhythm colours,
// so the app can write an image for one file or for a whole library without
// a browser. GDI+ does the text and the PNG; it is in every Windows.
//
// Header only, Windows only, and free of project references.
...
#include <cmath>
#include <windows.h>
// gdiplus.h spells min and max unqualified, which NOMINMAX takes away.
using std::max;
using std::min;
#include <gdiplus.h>
...
// The page's own two settings that shape the picture. The page stores them as
// fontSize and lineHeight beside the style options.
...
// A run of one colour and weight; a word is a chord with its separator, never
// broken across lines; a line is what the sheet breaks, then wrapped to width.
...
inline std::vector<ImageLine> ImageLines(const StyledResult& r) {
    ...
    for (size_t i = 0; i < r.items.size(); ++i) {
        ...
        if (item.kind == Kind::Chord) {
            ...
            // As the text has it: a blank line, the comment, a blank line.
            blank();
            lines.emplace_back();   // a blank line
    ...
    // Leading, trailing and doubled blank lines go, as TidyLines takes them
    // out of the text.
...
// Writes the sheet as a PNG and returns its size in pixels. scale is how many
// pixels a CSS pixel gets, two by default so text stays sharp when the picture
// is enlarged; maxWidth is the wrapping width in CSS pixels, the page's usual
// column. Throws when the file cannot be written.
inline std::pair<int, int> SavePng(const StyledResult& r, const std::string& title, const Look& look,
    ...
    // Measure with a throwaway surface, then lay the lines out.
    ...
    if (bitmap.Save(file.c_str(), &png, nullptr) != Gdiplus::Ok) throw std::runtime_error("Cannot write " + file.string() + ".");
    return {width, height};
}
