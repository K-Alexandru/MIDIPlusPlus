...
// The editor owns a smooth curve; the 32 thresholds are only its output format.
...
struct VelocityEdit {
    ...
    float sensitivity = 0;
    float contrast = 0;
    // Empty means the selected preset plus its macro adjustments. Once the
    // graph is touched, anchors become the single editable representation for
    // both point editing and free draw.
...
inline std::vector<VelocityPoint> VelocityLegalAnchors(std::vector<VelocityPoint> points) {
    ...
    // A stroke can revisit the same input. The last observation is the one the
    // hand left there, so collapse coincident points to it.
    ...
    // Pool-adjacent-violators is the least-squares non-decreasing repair. It
    // legalises adversarial anchors without pinning the cursor to a neighbour.
...
// VelocityLegalAnchors makes this monotone even when callers supply anchors in
// an arbitrary order or with descending output values.
...
inline size_t VelocityMoveAnchor(std::vector<VelocityPoint>& anchors, size_t index, VelocityPoint point) {
    ...
    // Keep the dragged point under the hand and move conflicting neighbours.
...
inline std::vector<VelocityPoint> VelocityApplySweep(const std::vector<VelocityPoint>& suppliedBase,
    ...
    // A light symmetric pass happens once, on release. During the gesture the
    // UI draws the raw stroke, so the line remains directly under the hand.
    ...
    // Repair only the replaced interval. The fixed join values prevent a dip
    // from changing either untouched side of the original curve.
